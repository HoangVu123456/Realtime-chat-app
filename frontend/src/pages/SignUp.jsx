...
import { EyeOff, KeyRound, Mail, MessageSquare, User, Eye, Loader2 } from 'lucide-react';
...
const SignUp = () => {
    ...
    const { is_signup, signup } = useAuthstore();
    const validate_form = () => {
        if (!data.fullname.trim()) return toast.error("Full name required!");
        if (!data.username.trim()) return toast.error("Username required!");
                ...
                <form onSubmit={handle_submit} className="space-y-6">
                    ...
                    <div className="flex w-full flex-col">
                        <div className="divider">
                            <button type="submit" className="btn btn-wide cursor-pointer bg-primary/20" disabled={is_signup}>
                            {is_signup ? (<><Loader2 className="size-5 animate-spin"/>Loading...</>):("Create Account")}
                            </button>
                        </div>
                    </div>
